...
class ConnectViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        headerLabel.text = "Ingrese el código PIN para \n" + currentDeviceName
    }
    ...
    @IBAction func nextBtnClicked(_: Any) {
        ...
        Utility.showLoader(message: "Conectando al Interruptor", view: view)
            ...
            DispatchQueue.main.async {
                ...
                case let .failedToConnect(error):
                    ...
                    case .sessionInitError:
                        self.showStatusScreen(step1Failed: true, message: error.description + "Chequear si el PIN es correcto.")
                    default:
                        self.showStatusScreen(step1Failed: true, message: error.description)
                    }
                default:
                    let action = UIAlertAction(title: "Reintentar", style: .default, handler: nil)
                    self.showAlert(error: "Interruptor desconectado", action: action)
                }
            }
        }
    }
