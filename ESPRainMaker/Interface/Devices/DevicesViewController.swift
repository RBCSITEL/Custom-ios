...
class DevicesViewController: UIViewController {
    ...
    private func setViewForNoNodes() {
        if User.shared.associatedNodeList?.count == 0 || User.shared.associatedNodeList == nil {
            infoLabel.text = "No hay Interruptores agregados"
    ...
    private func setupSegmentControl() {
        ...
        for i in 0 ... NodeGroupManager.shared.nodeGroups.count {
            ...
            if i == 0 {
                stringBoundingbox = "Todos los Interruptores".size(withAttributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 17.5, weight: .semibold)])
                segmentControl.insertSegment(withTitle: "Todos los Interruptores", at: i, animated: false)
            } else {
    ...
    private func configureSegmentControl() {
        ...
        let allDeviceSize = getFontWidthForString(text: "Todos los Interruptores")
    ...
    private func unhideInitialView(error: ESPNetworkError?) {
        User.shared.associatedNodeList = localStorageHandler.fetchNodeDetails()
        if User.shared.associatedNodeList?.count == 0 || User.shared.associatedNodeList == nil {
            infoLabel.text = "No hay Interruptores\n" + (error?.description ?? "Algo salió mal!!")
            ...
            collectionView.isHidden = false
            addButton.isHidden = false
            Utility.showToastMessage(view: view, message: "Error de red: \(error?.description ?? "Algo ha fallado!!")")
        }
        if Configuration.shared.appConfiguration.supportGrouping {
            NodeGroupManager.shared.nodeGroups = localStorageHandler.fetchNodeGroups() ?? []
            setupSegmentControl()
