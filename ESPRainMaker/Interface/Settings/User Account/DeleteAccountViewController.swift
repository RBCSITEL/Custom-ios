...
class DeleteAccountViewController: UIViewController {
    ...
    @IBAction func confirmDeletePressed(_ sender: Any) {
        view.endEditing(true)
        Utility.showLoader(message: "Borrando cuenta...", view: self.view)
        if let verificationCode = confirmationCodeTextField.text {
            apiManager.genericAuthorizedJSONRequest(url: Constants.deleteUserAccount + "?verification_code=" + verificationCode, parameter: nil, method: .delete) { response, error in
                ...
                if let statusJSON = response as? [String: Any], let status = statusJSON["status"] as? String {
                    ...
                    Utility.showToastMessage(view: self.view, message: description ?? "Imposible verificar solicitud.", duration: 5.0)
            ...
            Utility.showToastMessage(view: self.view, message: "Ingrese código de verificación                      .", duration: 5.0)
        }
    }
    ...
    @IBAction func deleteAccountPressed(_ sender: Any) {
        ...
        let confirmAction = UIAlertAction(title: "Proceder", style: .destructive) { _ in
    ...
    private func showSuccessAlert() {
        let alertController = UIAlertController(title: "", message: "Su cuenta fue borrada exitosamente!", preferredStyle: .alert)
