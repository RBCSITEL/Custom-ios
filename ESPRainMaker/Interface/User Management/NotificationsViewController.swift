    ...
    private func getAddSharingNotificationTableViewCell(for indexPath:IndexPath) -> AddSharingNotificationTableViewCell {
        ...
        if let deviceList = request.metadata, deviceList.count > 0 {
            if deviceList.count == 1 {
                cell.detailLabel.text = primaryUserID + " quiere compartir interruptor \(deviceList[0])."
            } else {
                var devices = ""
                ...
                cell.detailLabel.text = primaryUserID + " quiere compartir interruptor \(devices) "
            }
        } else {
            cell.detailLabel.text = primaryUserID + " quiere compartir nodo \(nodeIDs ?? "") "
        }
