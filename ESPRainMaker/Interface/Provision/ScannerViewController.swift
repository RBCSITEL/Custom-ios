        ...
        ESPProvisionManager.shared.scanQRCode(scanView: scannerView) { espDevice, scanError in
            if let device = espDevice {
                if self.isDeviceSupported(device: device) {
                    Utility.showLoader(message: "Conectando a Interrutpor", view: self.view)
                    ...
                    DispatchQueue.main.async {
                        self.retry(message: "Dispositivo no soportado.")
                    }
                }
            } else {
                DispatchQueue.main.async {
                    if let error = scanError {
                        ...
                        case .invalidQRCode(let code):
                            let sanitizedJsonString = code.replacingOccurrences(of: "'", with: "\"")
                            if let jsonData = sanitizedJsonString.data(using: .utf8) {
                                do {
                                    let decoder = JSONDecoder()
                                    let decodeResponse = try decoder.decode(ESPScanResultCopy.self, from: jsonData)
                                    ESPProvisionManager.shared.createESPDevice(deviceName: decodeResponse.name, transport: decodeResponse.transport, proofOfPossession: decodeResponse.pop) { espDevice, error in
                                        if let device = espDevice {
                                            if self.isDeviceSupported(device: device) {
                                                Utility.showLoader(message: "Conectando a Interruptor", view: self.view)
                                                switch device.transport {
                                                    case .ble:
                                                        self.connectDevice(espDevice: device)
                                                    case .softap:
                                            ...
                                            self.retry(message: "Interruptor no encontrado.Intente nuevamente")
                                        }
                                    }
                                } catch {
                                    self.retry(message: error.localizedDescription)
                                }
                            } else {
                                self.retry(message: error.description)
                            }
                        ...
                        case .videoInputError, .videoOutputError, .cameraNotAvailable, .avCaptureDeviceInputError:
                            self.showAlertWith(message: "Imposible escanear código QR. Error con la cámara del dispositivo móvil")
                        }
                    }
                }
            }
            ...
            switch status {
            case .readingCode:
                Utility.showLoader(message: "Buscando QR", view: self.view)
            case .searchingBLE(let device):
                Utility.showLoader(message: "Buscando Interruptor Bluetooth: \(device)", view: self.view)
            case .joiningSoftAP(let device):
                Utility.showLoader(message: "Uniendo a Red: \(device)", view: self.view)
            default:
                break
            }
        }
    ...
    func connectDevice(espDevice: ESPDevice) {
        espDevice.connect(delegate: self) { status in
            DispatchQueue.main.async {
                ...
                default:
                    self.retry(message: "Interruptor no pudo ser conectado. Intente nuevamente")
                }
            }
        }
    }
...
struct ESPScanResultCopy: Decodable {
    var name: String
    var username: String?
    var pop: String?
    ...
    init(from decoder: Decoder) throws {
        ...
        security = try ESPSecurity.init(rawValue: container.decodeIfPresent(Int.self, forKey: .security) ?? 1) ?? .secure
        ...
        case "softap":
            transport = .softap
        default:
            throw ESPScanErrorCopy.invalidQRCode
        }
        password = try container.decodeIfPresent(String.self, forKey: .password)
    }
}

enum ESPScanErrorCopy: Error {
    case invalidQRCode
}
