...
class ScheduleListViewController: UIViewController {
    ...
    @IBAction func editTableView(_ sender: UIButton) {
        ...
        sender.setTitle(tableView.isEditing ? "Hecho" : "Editar", for: .normal)
    }
    ...
    @IBAction func refreshScheduleList(_: Any) {
        ...
        NetworkManager.shared.getNodes { nodes, error in
            ...
            if error != nil {
                DispatchQueue.main.async {
                    Utility.showToastMessage(view: self.view, message: "Error de red: \(error?.description ?? "Algo ha fallado!!")")
                }
            } else {
    ...
    func showScheduleList() {
        ...
        if ESPScheduler.shared.availableDevices.count < 1 {
            ...
            initialLabel.text = "No hay interruptores \n que soporten esto."
            ...
            initialLabel.text = "Sin Horarios agregados."
        } else {
...
extension ScheduleListViewController: UITableViewDelegate {
    ...
    func tableView(_: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            let alertController = UIAlertController(title: "Está seguro?", message: "", preferredStyle: .alert)
            let cancelAction = UIAlertAction(title: "No", style: .default, handler: nil)
            let confirmAction = UIAlertAction(title: "Si", style: .destructive) { _ in
                DispatchQueue.main.async {
                    ...
                    ESPScheduler.shared.deleteScheduleAt(key: self.scheduleList[indexPath.row/2], onView: self.view) { result  in
                        ...
                        case .success(let nodesFailed):
                            DispatchQueue.main.asyncAfter(deadline: .now()+1.0, execute: {
                                if !nodesFailed {
                                    Utility.showToastMessage(view: self.view, message: "Horario eliminado exitosamente.", duration: 2.0)
                                }
...
extension ScheduleListViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if schedule.trigger.days == 0 {
            cell.daysLabel.text = "Una vez"
        } else if schedule.trigger.days == 127 {
            cell.daysLabel.text = "Todos los días"
        } else {
            let dayDescription = schedule.week.getShortDescription()
            if dayDescription.lowercased() == "weekends" || dayDescription.lowercased() == "todos los dias" {
