...
class SignInViewController: UIViewController, ESPNoRefreshTokenLogic, UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let regularText = NSMutableAttributedString(string: "Al proceder, confirmo que he leido y aceptado la ", attributes: [ NSAttributedString.Key.foregroundColor: UIColor.black, NSAttributedString.Key.font: UIFont.systemFont(ofSize: 15)])
        ...
        let privacyText = NSMutableAttributedString(string: "Politica de Privacidad")
        ...
        regularText.append(NSMutableAttributedString(string: " y ", attributes: [ NSAttributedString.Key.foregroundColor: UIColor.black, NSAttributedString.Key.font: UIFont.systemFont(ofSize: 15)]))
        let termsOfUseText = NSMutableAttributedString(string: "Términos de Uso.")
    ...
    @IBAction func signInPressed(_: AnyObject) {
        ...
        if ESPNetworkMonitor.shared.isConnectedToNetwork {
            guard let usernameValue = username.text, !usernameValue.isEmpty, let password = password.text, !password.isEmpty else {
                let alertController = UIAlertController(title: "Información vacía",
                                                        message: "Por favor ingrese Nombre de Usuario y Contraseña válidos",
                                                        preferredStyle: .alert)
                let retryAction = UIAlertAction(title: "Reintentar", style: .default, handler: nil)
    ...
    func signIn(username: String, password: String) {
        Utility.showLoader(message: "Iniciando Sesión", view: view)
    ...
    func showAlert() {
        let alertController = UIAlertController(title: "Falla",
                                                message: "Error al iniciar sesión. Reintentar.",
                                                preferredStyle: .alert)
        let retryAction = UIAlertAction(title: "Reintentar", style: .default, handler: nil)
        alertController.addAction(retryAction)
        present(alertController, animated: true, completion: nil)
    ...
    @IBAction func signUp(_ sender: AnyObject) {
        ...
        if ESPNetworkMonitor.shared.isConnectedToNetwork {
            guard let userNameValue = email.text, !userNameValue.isEmpty,
                ...
                let alertController = UIAlertController(title: "Info requerida vacía",
                                                        message: "Usuario / Contraseña son reuqeridos para registrarse.",
                                                        preferredStyle: .alert)
            ...
            if !checked {
                let alertController = UIAlertController(title: "Error!!",
                                                        message: "Para proceder, por favor acepte las Políticas de Privacidad y Términos de Uso.",
                                                        preferredStyle: .alert)
                let okAction = UIAlertAction(title: "Ok", style: .default, handler: nil)
                alertController.addAction(okAction)
                present(alertController, animated: true, completion: nil)
