...
class EditNodeGroupViewController: UIViewController {
    ...
    @IBAction func removeGroupButtonPressed(_: Any) {
        ...
        let alertController = UIAlertController(title: "Remover", message: "Está seguro en remover Grupo?", preferredStyle: .alert)
        let cancelAction = UIAlertAction(title: "No", style: .default, handler: nil)
        let confirmAction = UIAlertAction(title: "Si", style: .destructive) { _ in
            Utility.showLoader(message: "Removiendo Grupo...", view: self.view)
    ...
    @IBAction func nameButtonPressed(_: Any) {
        ...
        let input = UIAlertController(title: "Entrar nuevo nombre", message: "", preferredStyle: .alert)
        ...
        input.addAction(UIAlertAction(title: "Renombrar", style: .default, handler: { [weak input] _ in
            let textField = input?.textFields![0]
            guard let name = textField?.text, !name.trimmingCharacters(in: .whitespaces).isEmpty else {
                return
            }
            Utility.showLoader(message: "Renombrando...", view: self.view)
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        case UICollectionView.elementKindSectionHeader:
            ...
            headerView.selectButtonAction = {
                Utility.showLoader(message: "Removiendo Interruptor..", view: self.view)
