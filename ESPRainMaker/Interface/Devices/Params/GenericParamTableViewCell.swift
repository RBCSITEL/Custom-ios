...
class GenericParamTableViewCell: GenericControlTableViewCell {
    ...
    @IBAction override func editButtonTapped(_: Any) {
        ...
        if param?.type == Constants.deviceNameParam {
            input = UIAlertController(title: attributeKey, message: "Ingresar nombre de Interruptor de longitud 1-32 caracteres", preferredStyle: .alert)
        } else {
            input = UIAlertController(title: attributeKey, message: "Ingresar nuevo valor", preferredStyle: .alert)
        }
        ...
        input.addAction(UIAlertAction(title: "Actualizar", style: .default, handler: { [weak input] _ in
            ...
            if dataType.lowercased() == "int" {
                ...
                if param?.type == Constants.deviceNameParam {
                    if value.count < 1 || value.count > 32 || value.isEmpty || value.trimmingCharacters(in: .whitespaces).isEmpty {
                        showAlert(message: "Ingrese un nombre válido de longitud de 1-32 caracteres")
                        return
                    }
                }
