...
class NodeDetailsViewController: UIViewController {
    ...
    @IBAction func deleteNode(_: Any) {
        ...
        if currentNode.devices!.count > 1 {
            title = "Está seguro?"
            message = "Todos los Interruptores asociados serán eliminados"
        }
        ...
        let yesAction = UIAlertAction(title: "Si", style: .destructive) { _ in
            Utility.showLoader(message: "Borrando nodo", view: self.view)
            ...
            NetworkManager.shared.addDeviceToUser(parameter: parameters) { _, error in
                ...
                DispatchQueue.main.async {
                    ...
                    if !ESPNetworkMonitor.shared.isConnectedToNetwork {
                        Utility.showToastMessage(view: self.view, message: "Imposible remover Interrutor, por favor revisar la conexión a internet.")
                    } else {
    ...
    func createDataSource() {
        ...
        var index = 0
        dataSource = [[]]
        dataSource[index].append("ID de Nodo")
        dataSource[index].append("ID de Nodo:\(currentNode.node_id ?? "")")

        // Second section will contain node information
        index += 1
        dataSource.append([])
        dataSource[index].append("Información de Nodo")
        dataSource[index].append("Tipo:\(currentNode.info?.type ?? "")")
        ...
        if let tzService = currentNode.services?.first(where: { $0.type == Constants.timezoneServiceName }) {
            if let tzParam = tzService.params?.first(where: { $0.type == Constants.timezoneServiceParam }) {
                ...
                dataSource[index].append("Zona Horaria:\(timezone ?? "")")
            }
        }
        ...
        if Configuration.shared.appConfiguration.supportSharing {
            ...
            sharingIndex = index
            dataSource.append([])
            dataSource[index].append("Compartiendo")
            if let primaryUsers = currentNode.primary {
                if primaryUsers.contains(User.shared.userInfo.email) {
                    ...
                    dataSource[index][0] = "Compartido con"
                    ...
                    dataSource[index].append("Agregar Miembro")
    ...
    private func getSharingInfo() {
        ...
        NodeSharingManager.shared.getSharingDetails(node: currentNode) { error in
            ...
            if error != nil {
                ...
                Utility.showToastMessage(view: self.view, message: "Imposible actualizar información del Interruptor:\(error!.description).")
                return
            }
                ...
                if let sharingRequests = requests {
                    ...
                    DispatchQueue.main.async {
                        ...
                        if self.pendingRequests.count > 0 {
                            if !self.dataSource[self.sharingIndex].contains("Pendiente de aceptación") {
                                self.dataSource[self.sharingIndex].append("Pendiente de aceptacion")
                                self.tableView.reloadData()
                            }
                        }
                    }
    ...
    private func getTableViewCellForAddMember(forIndexPath: IndexPath) -> UITableViewCell {
        ...
        cell.addMemberButtonAction = {
            ...
            let input = UIAlertController(title: "Agregar Miembro", message: "", preferredStyle: .alert)
            input.addTextField { textField in
                textField.placeholder = "Ingresar email"
                textField.keyboardType = .emailAddress
                textField.delegate = self
                textField.addTarget(self, action: #selector(self.textFieldDidChange(textField:)), for: .editingChanged)
            }
            input.addAction(UIAlertAction(title: "Cancelar", style: .destructive, handler: { _ in

            }))

            let shareAction = UIAlertAction(title: "Compartir", style: .default, handler: { [weak input] _ in
                let textField = input?.textFields![0]
                guard let email = textField?.text, !email.trimmingCharacters(in: .whitespaces).isEmpty else {
                    return
                }
                Utility.showLoader(message: "Compartir en progreso..", view: self.view)
                NodeSharingManager.shared.createSharingRequest(userName: email, node: self.currentNode) { request, error in
                    ...
                    guard let apiError = error else {
                        DispatchQueue.main.async {
                            if self.pendingRequests.count < 1 {
                                self.dataSource[self.sharingIndex].append("Pendiente de aceptación")
                            }
                            self.pendingRequests.append(request!)
                            self.tableView.reloadSections(IndexSet(arrayLiteral: self.sharingIndex), with: .automatic)
                        }
                    ...
                    DispatchQueue.main.async {
                        self.view.makeToast("Falla al compartir nodo, con error: \(apiError.description)", duration: 5.0, position: ToastManager.shared.position, title: nil, image: nil, style: ToastManager.shared.style, completion: nil)
                    }
                }
            })
    ...
    private func getTableViewCellForPendingRequest(forIndexPath: IndexPath) -> UITableViewCell {
        ...
        cell.removeButtonAction = {
            Utility.showLoader(message: "Removiendo solicitud..", view: self.view)
            NodeSharingManager.shared.deleteSharingRequest(request: request) { _, error in
                ...
                DispatchQueue.main.async {
                    Utility.showToastMessage(view: self.view, message: "Falla al eliminar pedido de compartir Interruptor: \(apiError.description)")
                }
            }
        }
        ...
        if let timestamp = request.request_timestamp {
            ...
            var expiringText = "Expira hoy"
            let daysLeft = 7 - days
            if daysLeft > 1 {
                expiringText = "Expira en \(daysLeft) dias"
            } else if daysLeft == 1 {
                expiringText = "Expirea en 1 dia"
            } else {
                expiringText = "Expira hoy"
            }
...
extension NodeDetailsViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if headerTitle == systemServices, !currentNode.isConnected {
            headerView.headerLabel.text = headerTitle + " (Fuera de Línea)"
        }
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if pendingRequests.count > 0 {
            if indexPath.section == sharingIndex, indexPath.row + 1 == dataSource[sharingIndex].count - 1 {
                if dataSource[sharingIndex][indexPath.row + 1] == "Pendiente de aceptación" {
                    return 40.0
                }
            }
        }
            ...
            if indexPath.section == sharingIndex {
                ...
                case "Compartido con":
                    ...
                    default:
                        ...
                        cell.removeMemberButton.isHidden = false
                        cell.removeButtonAction = {
                            Utility.showLoader(message: "Removiendo usuario..", view: self.view)
                            NodeSharingManager.shared.deleteSharing(forNode: self.currentNode, email: cell.secondaryUserLabel.text ?? "") { success, error in
                                ...
                                guard let apiError = error else {
                                    if success {
                                        ...
                                        DispatchQueue.main.async {
                                            Utility.showToastMessage(view: self.view, message: "Falla al eliminar solicitud de compartir: Error desconocido.")
                                        }
                                    }
                                    return
                                }
                                DispatchQueue.main.async {
                                    Utility.showToastMessage(view: self.view, message: "Falla al eliminar solicitud de compartir Interruptor: \(apiError.description)")
                                }
                            }
                        }
