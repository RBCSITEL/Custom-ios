...
class ScheduleViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if ESPScheduler.shared.currentSchedule.id != nil {
            ...
            removeButton.setTitle("Remover", for: .normal)
    ...
    @IBAction func scheduleNamePressed(_: Any) {
        ...
        input.addAction(UIAlertAction(title: "Hecho", style: .default, handler: { [weak input] _ in
                        ...
                        ESPScheduler.shared.shouldEnableSchedule(onView: self.view, completionHandler: { result in
                            DispatchQueue.main.asyncAfter(deadline: .now()+1.0, execute: {
                                ...
                                case .success(let nodesFailed):
                                    ...
                                    break
                                case .failure:
                                    Utility.showToastMessage(view: self.view, message: "Falla al agregar Horario. Chequear su conexión!!")
                                }
                                self.navigationController?.popToRootViewController(animated: false)
                            })
                        })
